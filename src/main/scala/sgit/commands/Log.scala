...
object Log {
  ...
  def log(userPath: String): Unit =
    {
      // We get the path of our project
      val projectDir = userPath + "/.sgit/"
  ...
  def createLog(shaValue: String, commitMessage: String, committerName: String, shaSubCommit: String
                ,userPath: String): Unit =
    {
      // We get the path of our project
      val pathDir = userPath + "/.sgit/"
      ...
      FileApi.utilWriter(pathFile, "Branch: " + SgitApi.getBranchFile(userPath).getName)
  ...
  def logP(userPath: String): Unit =
    {
      // We get all of our commits from our log file and reverse it to have our newest at the beginning
      val logContent = FileApi.listFromFile(userPath + "/.sgit/log/log",0).grouped(6).toList
      ...
      logContentUniqueSha.foreach(x => SgitApi.diffBetweenCommits(x, userPath))
    }
}
